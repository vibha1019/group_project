...
<script>
    ...
    const minHamsterPosition = 0;
const maxHamsterPosition = document.body.clientWidth - hamster.clientWidth; // Adjust based on hamster size

function moveHamster(direction) {
    const step = 45;

    if (direction === 'left') {
        hamsterPosition = Math.max(hamsterPosition - step, minHamsterPosition);
    } else if (direction === 'right') {
        hamsterPosition = Math.min(hamsterPosition + step, maxHamsterPosition);
    }

    hamster.style.transform = `translateX(${hamsterPosition}px)`;
}
let hamsterPosition = (document.body.clientWidth - hamster.clientWidth) / 2; // Initial position at the center
