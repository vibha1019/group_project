...
<body>
    ...
    <script>
        ...
        function setRandomPosition(fallingItem) {
        const randomX = Math.floor(Math.random() * window.innerWidth);
        const randomY = -30; // Starts above the top of the screen
        falllingitem.style.left = `${randomX}px`;
        fallingItem.style.top = `${randomY}px`;
    }

   function fall(fallingItem) {
        const speed = Math.floor(Math.random() * 5) + 3; // Random speed between 3 and 7
        const interval = setInterval(function () {
            const currentTop = parseInt(fallingItem.style.top, 10);
            ...
            if (newTop > window.innerHeight) {
                ...
                setRandomPosition(fallingItem); // Reset position for the next fall
                fall(fallingItem); // Start falling again
            }
        }, 30); // Update every 30 milliseconds
    }   

    const numberOffallingItems = 10;
    for (let i = 0; i < numberOffallingItems; i++) {
        const fallingItem = createfallingItem();
        setRandomPosition(fallingItem);
        fall(fallingItem);
    }
